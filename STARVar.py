...
###Uploaded_variation     Location        Allele  Gene    Feature Feature_type    Consequence     cDNA_position   CDS_position    Protein_position        Amino_acids     Codons  Existing_variation      IMPACT  DISTANCE        STRAND  FLAGS   SYMBOL  SYMBOL_SOURCE   HGNC_ID BIOTYPE CANONICAL       TSL     SOURCE  SIFT    PolyPhen        EXON    INTRON  HGVSc   HGVSp   HGVS_OFFSET     AF      gnomAD_AF       gnomAD_AFR_AF   gnomAD_AMR_AF   gnomAD_ASJ_AF   gnomAD_EAS_AF   gnomAD_FIN_AF   gnomAD_NFE_AF   gnomAD_OTH_AF   gnomAD_SAS_AF   CLIN_SIG        SOMATIC PHENO   PAVS    GO_CLASSES      PHENOTYPE       PPI
    ...
    with open(fname, 'r') as f:
       ...
       for line in f:
         ...
         if ((len(lst)>5) and (lst[0] not in "#Uploaded_variation") and (lst[21] == "YES")):
           cnt=cnt+1
           vcf[cnt]=line.strip()
...
if sys.argv[1] =="":
   ...
   sys.exit(0)
if sys.argv[2] != "s" and sys.argv[2] !="p":
   print ("provide an option for genomic evidence based model, s for SIFT, p for PolyPhen-2. USAGE: python prod.py inputfile option symptoms")
   sys.exit(0)
...
print("Start of loading phenotype-pmid associations: Current Time =", current_time)

load_phenotypes()

now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("End of loading phenotype-pmid associations: Current Time =", current_time)

now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("Start of downloading phenotypes-pmid associations: Current Time =", current_time)
...
now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("End of downloading phenotypes-pmid associations: Current Time =", current_time)

now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("Start of loading variant-pmid associations: Current Time =", current_time)

load_variants()

now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("End of loading variant-pmid associations: Current Time =", current_time)

print("Start of loading gene-pmid associations: Current Time =", current_time)
load_genes()
now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("End of loading gene-pmid associations: Current Time =", current_time)

now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("Start of reading VCF file: Current Time =", current_time)
infile=sys.argv[1]
readfromfile(infile)
now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("End of reading VCF file: Current Time =", current_time)
...
now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("End of prediction: Current Time =", current_time)

polyphen_data_list= []
logreg_data_list = []
sift_data_list=[]

print("Start of combining scores and ranking variants: Current Time =", current_time)
...
now = datetime.now()
current_time = now.strftime("%H:%M:%S")
print("All processes are completed: Current Time =", current_time)
